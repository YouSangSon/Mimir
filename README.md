...
| **git-as-DB commit-back** | After collection, data is committed back to the repo (`concurrency.queue: max` guard + rebase) |
...
| Item | Value |
| :--- | :--- |
| **Tests** | 658 passing (adapters verified with recorded fixtures, no network) |
...
Detailed docs under `docs/` are Korean-first; the root README files are the maintained English/Korean/Chinese entry points.
