...
| 스펙 | 만드는 것 | 의존성 | 완료 시 결과물 | 난이도 |
|---|---|---|---|---|
| **S1 Collector** | 어댑터 수집기 + JSONL 저장 + 백필 + cron + 최소 가시성 | 없음 | 데이터가 매일 쌓이고 수집 동작을 눈으로 확인 | 중 |
| **S2 Analysis** | 지표·시그널 + ⭐별점 + 면책 | S1 데이터 | 별점 달린 인사이트 레코드 | 중상 |
| **S3 Delivery** | 텔레그램 정기 발송 + 풍부한 HTML 리포트 아카이브 | S2 | 매일 받아보는 리포트/알림 | 중 |
