...
> `build_sources`는 A3 이후 `SourceSpec` 등록 테이블을 순회하지만, 여전히 **API 키 유무로 소스를 조건부 생성**한다. 키가 빠지면 소스가 사라지고 — **닥터가 잡아야 할 바로 그 실패(데이터 부재)를 점검 자체가 조용히 건너뛴다.** "FRED 키 누락 → FRED 소스 없음 → FRED 신선도 점검 없음 → 데이터 없어도 OK"는 무음 실패다.
