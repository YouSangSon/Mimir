...
| 기능 | 동작 |
| :--- | :--- |
| **소스 어댑터** | 공통 `Source` 프로토콜 뒤에 7개 내장 소스를 격리 구현하고, 외부 package는 `mimir.sources` entry point와 `sources.plugins.<source_id>` 설정 namespace로 source plugin 등록 |
...
| 항목 | 값 |
| :--- | :--- |
| **테스트** | 424 passing (어댑터는 녹화 픽스처로 네트워크 없이 검증) |
