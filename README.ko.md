...
#   config/sources.yaml        소스 on/off · GRAY 정책 · 시리즈/피드/종목별 RSS
...
| 기능 | 동작 |
| :--- | :--- |
| **소스 어댑터** | 공통 `Source` 프로토콜 뒤에 7개 내장 소스를 격리 구현하고, 외부 package는 `mimir.sources` entry point로 source plugin 등록 |
...
내장 소스 추가는 `sources/`에 어댑터 하나 + `SourceSpec` 등록 한 줄로 끝난다. 외부 package는 `mimir.sources` entry point로 `SourceSpec`을 등록할 수 있다. 신뢰한 plugin만 설치해야 한다. Plugin은 Mimir 프로세스 안에서 실행되고 sandbox되지 않으며, API key를 포함한 설정값을 받을 수 있다. 상위 레이어(분석·매매)는 저장된 envelope만 읽는다.
...
| 항목 | 값 |
| :--- | :--- |
| **테스트** | 415 passing (어댑터는 녹화 픽스처로 네트워크 없이 검증) |
...
| 영역 | 상태 |
| :--- | :--- |
| **인사이트/별점** | 규칙 기반 시그널로 구현됨. ⭐ 확신도, confidence, attention, 면책 문구를 포함한다. 뉴스 매칭은 종목별 RSS feed, 보수적 기본 회사명 alias, 사용자 alias를 함께 쓰며, LLM 감성 시그널은 off-by-default seam으로 제공. 자동 feed discovery/catalog는 아직 없다 |
