...
| 功能 | 行为 |
| :--- | :--- |
| **数据源适配器** | 在统一的 `Source` 协议背后隔离实现 7 个数据源（新增内置数据源 = 适配器文件 + `SourceSpec` 注册） |
...
新增内置数据源只需在 `sources/` 加一个适配器 + 一条 `SourceSpec` 注册即可完成，上层（分析·交易）只读取已存储的 envelope。
...
| 项目 | 值 |
| :--- | :--- |
| **测试** | 349 passing（适配器以录制的 fixture 在无网络下验证） |
| **覆盖率** | `mimir/` 97%（门槛 80%） |
