...
新增内置数据源只需在 `sources/` 加一个适配器 + 一条 `SourceSpec` 注册即可完成。外部 package 可以通过 `mimir.sources` entry point 注册 `SourceSpec`。上层（分析·交易）只读取已存储的 envelope。
...
| 项目 | 值 |
| :--- | :--- |
| **测试** | 396 passing（适配器以录制的 fixture 在无网络下验证） |
| **覆盖率** | `mimir/` 97%（门槛 80%） |
