...
def test_monthly_ecos_series_not_false_alarmed(tmp_path: Path):
    store = write_fresh_tree(tmp_path, NOW)
    # ECOS base rate (MONTHLY-registered) latest 20 days ago must be OK, not CRITICAL,
    # even though the macro dataset floor is DAILY.
    ecos_day = date(2026, 5, 26)  # 20 calendar days before 6/15
    write_partition(
        tmp_path, Dataset.MACRO, ecos_day,
        [
            make_record(
                Dataset.MACRO,
                ecos_day,
    ...
    report = run_doctor(store=store, watchlist=EMPTY_WATCHLIST, now=NOW)
    ecos = [f for f in report.findings if f.scope == "722Y001.0101000"]
    assert all(f.severity is Severity.OK for f in ecos)
    assert not [f for f in ecos if f.severity is Severity.CRITICAL]
...
def test_missing_macro_still_flags_missing_macro(tmp_path: Path):
    # Regression: the expected set comes from EXPECTED_DATASETS, not build_sources().
    # No macro data at all must still be CRITICAL missing.
